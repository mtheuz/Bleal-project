...
const Differentials = () => {
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="absolute top-1/2 left-0 w-1/4 sm:w-1/3 h-72 sm:h-96 blur-3xl"
      />
      <div
        aria-hidden="true"
        className="absolute top-1/2 right-0 w-1/4 sm:w-1/3 h-72 sm:h-96"
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        <header className="text-center mb-12 sm:mb-16">
          <h2
            ...
            className="text-2xl sm:text-4xl font-black mb-2 oswald"
          >
            NOSSOS DIFERENCIAIS
          </h2>
          <p className="text-xs text-white/70 max-w-2xl mx-auto mb-2 font-extralight uppercase">
            O que nos torna referência em produção de eventos e iluminação
            profissional
          </p>

          <div
            aria-hidden="true"
            className="flex gap-1 max-w-36 mx-auto"
          >
            {["bg-red-500", "bg-green-500", "bg-blue-500"].map((color, i) => (
              <div
                key={i}
                className={`w-24 sm:w-42 h-0.5 ${color} rounded-full pulse-rgb`}
              />
            ))}
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-12 items-center">
          <ul className="space-y-6 sm:space-y-8">
            {differentials.map((item, index) => {
              ...
              return (
                <li
                  key={index}
                  className="flex gap-4 sm:gap-6 p-4 sm:p-4 rounded-xl items-center bg-gray-800/30 border border-white/40 hover:border-zinc-300 transition-all duration-300 hover:scale-[1.03] sm:hover:translate-x-4 group animate-slide-up"
                ...
                >
                  <div className="flex-shrink-0 w-10 h-10 sm:w-14 sm:h-14 rounded-lg bg-black flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                    <Icon
                      aria-hidden="true"
                      className="w-5 h-5 sm:w-7 sm:h-7 text-zinc-500"
                    />
                  </div>
          ...
          <div
            className="relative h-72 sm:h-[600px] py-56 sm:py-0 border-2 border-white/40 rounded-2xl overflow-hidden animate-fade-in mt-8 md:mt-0"
            role="presentation"
          >
            <div className="absolute inset-0 flex items-center justify-center bg-zinc-900/50">
              <div className="text-center space-y-6 sm:space-y-2 p-10 sm:p-8">
                <p className="oswald text-5xl sm:text-8xl font-medium text-white">
                  +10.000
                </p>
                ...
                <div className="mt-10">
                  <div className="text-center border-2 border-white/50 bg-black rounded-2xl p-2">
                    <p className="text-2xl sm:text-4xl font-bold text-white mb-1 oswald">
                      +12
                    </p>
